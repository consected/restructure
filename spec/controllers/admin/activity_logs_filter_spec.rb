...
# Tests for the ActivityLogsController admin panel enhancements:
...
RSpec.describe Admin::ActivityLogsController, type: :controller do
  ...
  describe 'filtering by in_current_app_type' do
    before :all do
      create_admin
      create_user
      @app_type = create_app_type(name: "al_filter_test_#{rand(100_000)}", label: 'AL Filter Test')

      # Create a test user
      @test_user = User.create!(
        email: "al_filter_test_#{rand(100_000)}@test.com",
        first_name: 'ALFilter',
        last_name: 'Test',
        current_admin: @admin
      )
      ...
      @al_in_app = ActivityLog.active.first
      raise 'No active ActivityLog found for testing' unless @al_in_app

      # Add UAC for the activity log to associate it with the app type
      Admin::UserAccessControl.create!(
        app_type: @app_type,
        resource_type: :table,
        resource_name: @al_in_app.resource_name.pluralize,
        access: :read,
        current_admin: @admin
      )
    end
    ...
    it 'filters activity logs that are in the current app type when filter is yes' do
      ...
      expect(pm.pluck(:id)).to include(@al_in_app.id)
    end

    it 'filters activity logs that are NOT in the current app type when filter is no' do
      allow(@admin).to receive(:matching_user).and_return(@test_user)
      controller.instance_variable_set(:@current_admin, @admin)
      allow(controller).to receive(:current_admin).and_return(@admin)
      # Set the instance variable that filter_params populates
      ...
      pm = controller.send(:filtered_primary_model)
      expect(pm.pluck(:id)).not_to include(@al_in_app.id)
    end
  end
