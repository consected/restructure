...
RSpec.describe Tracker, type: :model do
  ...
  it 'does not update existing tracker record if attempting to insert with same protocol but the event date is earlier' do
    ...
    expect(t['sub_process_id']).to eq @sp1_2.id # the tracker record should still match the previous sub process
  ...
  it 'does not update the tracker item when an older tracker_history item is removed' do
    ...
    expect(t['sub_process_id']).to eq @sp2_1.id # the original (but event date more recent) item should remain
  ...
  it 'updates the tracker item when the most recent tracker_history item is removed' do
    ...
    expect(t['sub_process_id']).to eq @sp2_2.id # the original (but event date more recent) item should remain
  ...
  it 'updates most recent tracker_history record with new protocol_id through a delete and insert, generating a new trackers record' do
    ...
    expect(t['sub_process_id']).to eq @sp1_1.id # the original (but event date more recent) item should remain
    expect(t['protocol_id']).to eq @p1.id
