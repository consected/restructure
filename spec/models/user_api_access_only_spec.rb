...
#
# Test Coverage:
# - API-only user creation: bypasses interactive 2FA setup
# - two_factor_setup_required? returns false for API-only users
# - Non-API-only users retain existing 2FA creation behavior
# - Toggling api_access_only on an existing user bypasses interactive 2FA setup
...
# - Issue #1410: API-only users created or toggled while 2FA is disabled do not receive OTP state
...
RSpec.describe 'User api_access_only flag', type: :model do
  ...
  describe 'API-only user creation' do
    it 'does not require interactive 2FA setup when api_access_only is true' do
      user = User.create!(
    ...
    it 'does not generate OTP state when created as api_access_only while 2FA is disabled' do
      change_setting('TwoFactorAuthDisabledForUser', true)
      begin
        user = User.create!(
  ...
  describe 'toggling api_access_only on an existing user' do
    it 'does not require interactive 2FA setup when toggled to API-only' do
      user, = create_user
    ...
    it 'does not generate OTP state when toggled to API-only while 2FA is disabled' do
      change_setting('TwoFactorAuthDisabledForUser', true)
      begin
        user = User.create!(
    ...
    it 'does not require interactive 2FA setup when toggled off while 2FA is disabled' do
      user = User.create!(
  ...
  describe 'API-only user creation without explicit password (registration admin scenario)' do
    before :each do
      ...
      template_user = User.find_or_initialize_by(email: Settings::DefaultUserTemplateEmail)
      if template_user.new_record?
        template_user.assign_attributes(
          current_admin: @admin,
      ...
      template_user.update!(app_type:, current_admin: @admin) unless template_user.app_type == app_type
      unless template_user.user_roles.active.exists?(app_type:)
        template_user.user_roles.create!(current_admin: @admin, app_type:, role_name: 'registration template role')
      end
    end
