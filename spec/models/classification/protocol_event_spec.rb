...
# Verifies protocol event creation and active-scope behavior, including stable
# ordering semantics for returned definition records.
describe Classification::ProtocolEvent do
  ...
  describe 'definition' do
    ...
    it 'allows multiple Classification::Protocol Events to be created and returned in order based on name' do
      ...
      active_names = Classification::ProtocolEvent.active.pluck(:name)
      expected_names = Classification::ProtocolEvent.active.reorder(:name).pluck(:name)

      expect(active_names).to eq expected_names
      expect(active_names).not_to be_empty
    end
