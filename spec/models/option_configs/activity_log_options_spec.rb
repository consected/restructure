...
RSpec.describe 'Activity Log Options', type: :model do
  ...
  it 'uses calc_if correctly for the appropriate definition version when creating through associations' do

    # Create a player contact
    create_master
    create_sources 'player_contacts'
    setup_access :player_contacts, user: @master.current_user
    @player_contact = @master.player_contacts.create! data: "brian@test.com",
                                                    source: 'nflpa',
                                                    rank: 10,
                                                    rec_type: 'email'
    ...
    # Create an instance of the dynamic model (v1)
    sleep 2 # ensure there are no timing issues
    @dyn_instances[1] = @player_contact.activity_log__player_contact_emails.create! select_result: 'res1', extra_log_type: 'primary'
    expect(@dyn_instances[1].class.to_s).to eq dynamic_type.to_s
    ...
    # Create an instance of the dynamic model (v2)
    sleep 2 # ensure there are no timing issues
    al = @dyn_instances[2] = @player_contact.activity_log__player_contact_emails.create! select_result: 'good', extra_log_type: 'new_step', master: @master
    ...
    expect(dmdef.option_type_config_for(:new_step_2)).not_to be nil
    # Create an instance of the dynamic model (v2)
    sleep 2 # ensure there are no timing issues
    al = @dyn_instances[3] = @player_contact.activity_log__player_contact_emails.create! master: @master, select_result: 'bad', extra_log_type: 'new_step'
    expect(al.option_type_config.editable_if[:all][:this][:select_result]).to eq 'bad'
