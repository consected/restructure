...
RSpec.describe 'OptionConfigs::TriggerTypes definitions', type: :model do
  ...
  describe 'CreateFilestoreContainer' do
    ...
    it 'declares the expected allowed_keys' do
      expect(type_class.allowed_keys).to match_array(
        %i[name label create_with_role skip_if_exists if on_complete on_failure]
      )
    end
  ...
  describe 'Notify' do
    ...
    it 'declares type as :string_or_hash, role as :scalar_or_array_or_hash, and ignore_no_recipients as :boolean' do
      expect(type_class.key_type_rules[:type]).to eq(:string_or_hash)
      expect(type_class.key_type_rules[:role]).to eq(:scalar_or_array_or_hash)
  ...
  describe 'CreateReference' do
    ...
    it 'declares the expected allowed_keys' do
      expected = %i[if in force_create force_not_valid with_result with on_complete on_failure
                    to_existing_record this_has_no_master_association]
      expect(type_class.allowed_keys).to match_array(expected)
    end

    it 'declares force_create, force_not_valid and this_has_no_master_association as :boolean' do
      ...
      expect(type_class.key_type_rules[:this_has_no_master_association]).to eq(:boolean)
    end
  end
  ...
  describe 'PullExternalData' do
    ...
    it 'declares from and to as :hash (String is rejected by runtime)' do
      expect(type_class.key_type_rules[:from]).to eq(:hash)
      expect(type_class.key_type_rules[:to]).to eq(:hash)
    end
  end
  ...
  describe 'RedcapRequest' do
    ...
    it 'declares study, project_name, method as :string_or_hash and force_not_editable_save as :boolean' do
      ...
      expect(type_class.key_type_rules[:force_not_editable_save]).to eq(:boolean)
      expect(type_class.key_type_rules[:force_not_valid]).to eq(:boolean)
      expect(type_class.key_type_rules[:allow_empty_result]).to eq(:boolean)
      expect(type_class.key_type_rules[:response_code_field]).to eq(:string)
    end
  end
  ...
  describe 'validate_config' do
    ...
    describe 'named-entry validation' do
      # UpdateThis is a named-entry trigger: config is { label: { actual_keys... } }
      let(:type_class) { OptionConfigs::TriggerTypes::UpdateThis }

      it 'returns empty array for valid named entry config' do
        config = { my_update: { with: { status: 'active' } } }
        warnings = type_class.validate_config(config)
        expect(warnings).to be_an(Array)
        expect(warnings).to be_empty
      end
      ...
      it 'warns about unrecognized keys inside a named entry' do
        config = { my_update: { with: { status: 'active' }, unknown_key: 'bad' } }
        warnings = type_class.validate_config(config)
        expect(warnings).to be_present
