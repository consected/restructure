...
# Issue #1406 repro: dynamic definition `_constants` and config-level
# `set_variables` are lost in `background` save trigger blocks.
#
...
RSpec.describe 'Issue #1406 - stale cached definition in background job', type: :model do
  ...
  it 'refreshes config-level set_variables before running background triggers' do
    al_def = ActivityLog::PlayerContactPhone.definition
    ...
    al_def.extra_log_types = <<~END_DEF
      step_1:
        ...
        set_variables:
          - name: config_var
            value: from current definition
        save_trigger:
          on_save:
            - background:
                - log:
                    message: 'Background Config Variable = {{variables.config_var}}'
    END_DEF
    al_def.current_admin = @admin
    al_def.force_regenerate = true
    al_def.updated_at = DateTime.now + 1.second
    ...
    stale_config = @stale_worker_definition.option_type_config_for(:step_1)
    expect(stale_config.set_variables).to be_blank
    ...
    background_message = logged_messages.find { |msg| msg.include?('Background Config Variable') }
    expect(background_message).to include('from current definition')
  end
