...
class UpdateParticipantExitsView2 < ActiveRecord::Migration[5.2]
  ...
  def change
    ...
    sql = <<~END_SQL
      ...
      WHEN ps_interested1 = 'not interested' OR ps_interested2 = 'not interested' OR ps_still_interested = 'no'
        THEN 'not interested during phone screening'
      ...
      WHEN extra_log_type = 'perform_screening_follow_up' AND follow_up_still_interested = 'no'
        THEN 'not interested during screening follow-up'
      ...
      WHEN extra_log_type = 'on_hold' THEN 'on hold'
      ELSE extra_log_type
      END "status",
      dt.created_at "when"
      FROM
      ...
      OR
      (
        extra_log_type IN ('completed', 'exit_opt_out', 'exit_l2fu', 'exit_opt_out_covid19', 'on_hold')
      )
      OR
      (
