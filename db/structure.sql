...
CREATE VIEW ml_app.user_app_functional_groups AS
 WITH templates AS (
         ...
         SELECT ur.app_type_id,
           ...
           FROM (ml_app.user_roles ur
             JOIN templates t ON (((ur.user_id = t.user_id) AND (NOT COALESCE(ur.disabled, false)))))
        ), template_roles AS (
         ...
         SELECT ur.app_type_id,
           ...
           FROM (ml_app.user_roles ur
             JOIN ml_app.users u_1 ON ((ur.user_id = u_1.id)))
          WHERE (((u_1.email)::text !~~ '%template'::text) AND (NOT COALESCE(u_1.disabled, false)) AND (NOT COALESCE(ur.disabled, false)))
 ...
 SELECT DISTINCT urs.app_type_id,
   ...
   FROM ((((user_role_sets urs
     ...
     JOIN ml_app.users u ON ((urs.user_id = u.id)))
     JOIN ml_app.app_types a ON ((a.id = urs.app_type_id)))
     LEFT JOIN ml_app.role_descriptions rd ON ((((rd.role_template)::text = (tr.template_name)::text) AND (rd.app_type_id = urs.app_type_id))))
  ORDER BY urs.app_type_id, urs.user_id;
