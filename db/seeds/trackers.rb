module Seeds
  module Trackers
    ...
    def self.create_tracker_updates
      ...
      protocol.update(current_admin: Seeds.auto_admin) unless protocol.admin

      fu = protocol.sub_processes.find_or_initialize_by(name: 'flag updates')
      fu.update(current_admin: Seeds.auto_admin) unless fu.admin

      sub_process = protocol.sub_processes.find_or_initialize_by(name: 'record updates')
      sub_process.update(current_admin: Seeds.auto_admin) unless sub_process.admin
