...
CREATE FUNCTION persnet.find_new_remote_persnet_records() RETURNS TABLE(master_id integer, persnet_id bigint, container_id integer)
    LANGUAGE plpgsql
    AS $$
BEGIN
  RETURN QUERY
...
GRANT SELECT,INSERT,DELETE,UPDATE ON TABLE ipa_ops.adl_screener_data TO fphsetl;
