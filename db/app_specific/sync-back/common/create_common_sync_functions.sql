...
CREATE OR REPLACE FUNCTION lock_transfer_records_with_external_ids_and_events(from_db VARCHAR, to_db VARCHAR, master_ids INTEGER[], external_ids INTEGER[], external_type VARCHAR, events VARCHAR[], rec_updates timestamp without time zone[]) RETURNS INTEGER
LANGUAGE plpgsql
AS $$
BEGIN
