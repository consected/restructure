...
/*
  ...
  Run by the {{app_name}} sync scripts after a successful insert of Zeus player data into skeleton Elaine {{app_name}} subject record

*/
CREATE OR REPLACE FUNCTION activity_log_{{app_name}}_assignment_info_request_notification(activity_id INTEGER) RETURNS INTEGER
LANGUAGE plpgsql
AS $$
    DECLARE
        dl_users INTEGER[];
    ...
    BEGIN
        ...
        current_app_type_id := get_app_type_id_by_name('{{app_name}}');

        dl_users := get_user_ids_for_app_type_role(current_app_type_id, 'pi');

        SELECT * INTO activity_record FROM activity_log_{{app_name}}_assignments WHERE id = activity_id;

        IF activity_record.{{app_name}}_assignment_id IS NOT NULL AND activity_record.extra_log_type = 'primary'
        THEN
          ...
          INTO message_id
            create_message_notification_email(
              ...
              activity_record.id,
              'ActivityLog::{{AppName}}Assignment'::VARCHAR,
              activity_record.user_id,
              dl_users,
              '{{app_name}} notification layout'::VARCHAR,
              '{{app_name}} pi notification content'::VARCHAR,
              'New {{app_label}} Info Request'::VARCHAR,
              now()::TIMESTAMP
            )
          ;
...
/*
  ...
  Handle immediate notifications based on new {{app_name}} activity log records being created.
  Run from a trigger on insert into activity_log_{{app_name}}_assignments table

*/
DROP FUNCTION IF EXISTS activity_log_{{app_name}}_assignment_insert_notification() cascade;

CREATE OR REPLACE FUNCTION activity_log_{{app_name}}_assignment_insert_notification() RETURNS trigger
LANGUAGE plpgsql
AS $$
    DECLARE
      message_id INTEGER;
  ...
  BEGIN
        ...
        IF NEW.extra_log_type = 'contact_initiator' THEN
            ...
            INTO message_id
              create_message_notification_email(
                ...
                to_user_ids,
                '{{app_name}} notification layout'::VARCHAR,
                '{{app_name}} message notification content'::VARCHAR,
                '{{app_label}} contact from PI'::VARCHAR,
                now()::TIMESTAMP
              )
            ;
        ...
        IF NEW.extra_log_type = 'respond_to_pi' THEN
            ...
            INTO message_id
              create_message_notification_email(
                ...
                '{{app_name}} notification layout'::VARCHAR,
                '{{app_name}} message notification content'::VARCHAR,
                '{{app_label}} contact from RA'::VARCHAR,
                now()::TIMESTAMP
              );
        ...
        IF NEW.extra_log_type = 'primary' THEN
          ...
          IF num_primary_logs > 0 THEN
            PERFORM activity_log_{{app_name}}_assignment_info_request_notification(NEW.id);
          END IF;
        END IF;
...
CREATE TRIGGER activity_log_{{app_name}}_assignment_insert_notification AFTER INSERT ON activity_log_{{app_name}}_assignments FOR EACH ROW EXECUTE PROCEDURE activity_log_{{app_name}}_assignment_insert_notification();
