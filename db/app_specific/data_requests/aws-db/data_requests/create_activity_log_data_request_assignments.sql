...
CREATE OR REPLACE FUNCTION log_activity_log_data_request_assignment_update ()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  AS $$
BEGIN
...
ALTER TABLE ONLY activity_log_data_request_assignments
  ALTER COLUMN id SET DEFAULT NEXTVAL('activity_log_data_request_assignments_id_seq'::regclass);

ALTER TABLE ONLY activity_log_data_request_assignment_history
  ALTER COLUMN id SET DEFAULT NEXTVAL('activity_log_data_request_assignment_history_id_seq'::regclass);
...
CREATE TRIGGER activity_log_data_request_assignment_history_insert
  AFTER INSERT ON activity_log_data_request_assignments
  FOR EACH ROW
  EXECUTE PROCEDURE log_activity_log_data_request_assignment_update ();

CREATE TRIGGER activity_log_data_request_assignment_history_update
  AFTER UPDATE ON activity_log_data_request_assignments
  FOR EACH ROW
