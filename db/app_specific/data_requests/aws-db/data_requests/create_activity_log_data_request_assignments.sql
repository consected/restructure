...
-- table_generators/generate.sh activity_logs_table create activity_log_data_request_assignments data_request_assignment follow_up_date follow_up_time notes
      ...
      ALTER TABLE ONLY activity_log_data_request_assignments ALTER COLUMN id SET DEFAULT nextval('activity_log_data_request_assignments_id_seq'::regclass);
      ALTER TABLE ONLY activity_log_data_request_assignment_history ALTER COLUMN id SET DEFAULT nextval('activity_log_data_request_assignment_history_id_seq'::regclass);
      ...
      CREATE TRIGGER activity_log_data_request_assignment_history_insert AFTER INSERT ON activity_log_data_request_assignments FOR EACH ROW EXECUTE PROCEDURE log_activity_log_data_request_assignment_update();
      CREATE TRIGGER activity_log_data_request_assignment_history_update AFTER UPDATE ON activity_log_data_request_assignments FOR EACH ROW WHEN ((old.* IS DISTINCT FROM new.*)) EXECUTE PROCEDURE log_activity_log_data_request_assignment_update();
      ...
      ALTER TABLE ONLY activity_log_data_request_assignment_history
          ADD CONSTRAINT fk_activity_log_data_request_assignment_history_masters FOREIGN KEY (master_id) REFERENCES masters(id);

      ALTER TABLE ONLY activity_log_data_request_assignment_history
          ADD CONSTRAINT fk_activity_log_data_request_assignment_history_data_request_assignment_id FOREIGN KEY (data_request_assignment_id) REFERENCES data_request_assignments(id);
