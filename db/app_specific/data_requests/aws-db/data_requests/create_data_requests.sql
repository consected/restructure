...
CREATE OR REPLACE FUNCTION log_data_request_update ()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  AS $$
BEGIN
  ...
  RETURN NEW;
END;
  $$; CREATE TABLE data_request_history ( id integer NOT NULL,
  master_id integer,
  ...
  ALTER TABLE ONLY data_requests
    ALTER COLUMN id SET DEFAULT nextval('data_requests_id_seq'::regclass);
  ALTER TABLE ONLY data_request_history
    ALTER COLUMN id SET DEFAULT nextval('data_request_history_id_seq'::regclass);
  ALTER TABLE ONLY data_request_history
  ...
  CREATE TRIGGER data_request_history_insert
    AFTER INSERT ON data_requests
    FOR EACH ROW
    EXECUTE PROCEDURE log_data_request_update ( );
  CREATE TRIGGER data_request_history_update
