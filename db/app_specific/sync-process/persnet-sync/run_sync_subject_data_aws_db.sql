...
\copy temp_persnet_assignments (master_id, persnet_id, container_id) from $PERSNET_ASSIGNMENTS_FILE with (header true, format csv)
...
\copy (SELECT master_id, persnet_id, status, to_master_id FROM temp_persnet_assignments) TO $PERSNET_ASSIGNMENTS_RESULTS_FILE WITH (format csv, header true);
