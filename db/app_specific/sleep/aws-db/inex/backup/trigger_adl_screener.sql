...
BEGIN
    ...
    INTO matched_master_id
		...
		INNER JOIN sleep.sleep_assignments ipa
			ON m.id = ipa.master_id
		WHERE
      ipa.sleep_id = subject_id
    LIMIT 1
		;
...
create or replace function sleep.sync_new_adl_screener() returns trigger
language plpgsql
AS $$
  DECLARE
    matched_master_id INTEGER;
  ...
  BEGIN
    ...
    insert into sleep_adl_informant_screeners
      (
        master_id,
...
AFTER INSERT ON sleep_adl_screener_data
