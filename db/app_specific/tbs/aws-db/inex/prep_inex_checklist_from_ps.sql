...
DROP FUNCTION IF EXISTS tbs.activity_log_tbs_assignment_phone_screens_callback_set() CASCADE;

CREATE OR REPLACE FUNCTION tbs.activity_log_tbs_assignment_phone_screens_callback_set() RETURNS trigger
LANGUAGE plpgsql
AS $$
    DECLARE
  ...
  IF NEW.extra_log_type = 'finalize' THEN
    ...
    FROM tbs_ps_initial_screenings
    ...
    -- FROM tbs_ps_football_experiences
    -- WHERE master_id = NEW.master_id
    -- ORDER BY id DESC
    -- LIMIT 1;
    ...
    INSERT INTO tbs_inex_checklists
    (
      master_id,
      created_at,
      updated_at,
...
'Responded "' || health.cycle_blank_yes_no || '" to question "Are you able to sit on and pedal a bicycle?" in Health form.'
    ...
    INSERT INTO activity_log_tbs_assignment_inex_checklists
    (
      master_id,
      created_at,
      updated_at,
...
CREATE TRIGGER tbs_ps_to_inex AFTER INSERT ON tbs.activity_log_tbs_assignment_phone_screens FOR EACH ROW EXECUTE PROCEDURE activity_log_tbs_assignment_phone_screens_callback_set();
