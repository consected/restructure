BEGIN;
  ...
  create or replace view tbs.tbs_tms_reviews
  as select
    tms.id,
    tms.master_id,
    tms.user_id,
  ...
  from
    tbs_ps_tms_tests tms
  inner join
    tbs_ps_mris mri
  on tms.master_id = mri.master_id
  inner join
    tbs_ps_healths health
  on tms.master_id = health.master_id;


  GRANT SELECT ON tbs.tbs_tms_reviews TO fphs;
  GRANT SELECT ON tbs.tbs_tms_reviews TO fphsusr;
  GRANT SELECT ON tbs.tbs_tms_reviews TO fphsadm;
  GRANT SELECT ON tbs.tbs_tms_reviews TO fphsrailsapp;
