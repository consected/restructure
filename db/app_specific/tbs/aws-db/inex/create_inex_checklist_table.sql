...
-- table_generators/generate.sh create dynamic_models_table tbs_inex_checklists false fixed_checklist_type ix_consent_blank_yes_no ix_consent_details ix_not_pro_blank_yes_no ix_not_pro_details ix_age_range_blank_yes_no ix_age_range_details ix_weight_ok_blank_yes_no ix_weight_ok_details ix_no_seizure_blank_yes_no ix_no_seizure_details ix_no_device_impl_blank_yes_no ix_no_device_impl_details ix_no_ferromagnetic_impl_blank_yes_no ix_no_ferromagnetic_impl_details ix_diagnosed_sleep_apnea_blank_yes_no ix_diagnosed_sleep_apnea_details ix_diagnosed_heart_stroke_or_meds_blank_yes_no ix_diagnosed_heart_stroke_or_meds_details ix_chronic_pain_and_meds_blank_yes_no ix_chronic_pain_and_meds_details ix_tmoca_score_blank_yes_no ix_tmoca_score_details ix_no_hemophilia_blank_yes_no ix_no_hemophilia_details ix_raynauds_ok_blank_yes_no ix_raynauds_ok_details ix_mi_ok_blank_yes_no ix_mi_ok_details ix_bicycle_ok_blank_yes_no ix_bicycle_ok_details
      ...
      CREATE TABLE tbs_inex_checklist_history (
          id integer NOT NULL,
          master_id integer,
          ...
          tbs_inex_checklist_id integer
      );

      CREATE SEQUENCE tbs_inex_checklist_history_id_seq
          START WITH 1
          INCREMENT BY 1
          NO MINVALUE
          NO MAXVALUE
      ...
      ALTER SEQUENCE tbs_inex_checklist_history_id_seq OWNED BY tbs_inex_checklist_history.id;

      CREATE TABLE tbs_inex_checklists (
          id integer NOT NULL,
          master_id integer,
          fixed_checklist_type varchar,
          ix_consent_blank_yes_no varchar,
      ...
      CREATE SEQUENCE tbs_inex_checklists_id_seq
          START WITH 1
          INCREMENT BY 1
          NO MINVALUE
          NO MAXVALUE
      ...
      ALTER SEQUENCE tbs_inex_checklists_id_seq OWNED BY tbs_inex_checklists.id;

      ALTER TABLE ONLY tbs_inex_checklists ALTER COLUMN id SET DEFAULT nextval('tbs_inex_checklists_id_seq'::regclass);
      ALTER TABLE ONLY tbs_inex_checklist_history ALTER COLUMN id SET DEFAULT nextval('tbs_inex_checklist_history_id_seq'::regclass);

      ALTER TABLE ONLY tbs_inex_checklist_history
          ADD CONSTRAINT tbs_inex_checklist_history_pkey PRIMARY KEY (id);

      ALTER TABLE ONLY tbs_inex_checklists
          ADD CONSTRAINT tbs_inex_checklists_pkey PRIMARY KEY (id);

      CREATE INDEX index_tbs_inex_checklist_history_on_master_id ON tbs_inex_checklist_history USING btree (master_id);


      CREATE INDEX index_tbs_inex_checklist_history_on_tbs_inex_checklist_id ON tbs_inex_checklist_history USING btree (tbs_inex_checklist_id);
      CREATE INDEX index_tbs_inex_checklist_history_on_user_id ON tbs_inex_checklist_history USING btree (user_id);

      CREATE INDEX index_tbs_inex_checklists_on_master_id ON tbs_inex_checklists USING btree (master_id);

      CREATE INDEX index_tbs_inex_checklists_on_user_id ON tbs_inex_checklists USING btree (user_id);

      CREATE TRIGGER tbs_inex_checklist_history_insert AFTER INSERT ON tbs_inex_checklists FOR EACH ROW EXECUTE PROCEDURE log_tbs_inex_checklist_update();
      CREATE TRIGGER tbs_inex_checklist_history_update AFTER UPDATE ON tbs_inex_checklists FOR EACH ROW WHEN ((old.* IS DISTINCT FROM new.*)) EXECUTE PROCEDURE log_tbs_inex_checklist_update();
      ...
      ALTER TABLE ONLY tbs_inex_checklist_history
          ADD CONSTRAINT fk_tbs_inex_checklist_history_users FOREIGN KEY (user_id) REFERENCES users(id);

      ALTER TABLE ONLY tbs_inex_checklist_history
          ADD CONSTRAINT fk_tbs_inex_checklist_history_masters FOREIGN KEY (master_id) REFERENCES masters(id);




      ALTER TABLE ONLY tbs_inex_checklist_history
          ADD CONSTRAINT fk_tbs_inex_checklist_history_tbs_inex_checklists FOREIGN KEY (tbs_inex_checklist_id) REFERENCES tbs_inex_checklists(id);
