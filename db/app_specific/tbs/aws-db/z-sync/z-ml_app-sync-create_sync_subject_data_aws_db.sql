...
-------------------------------------------------------
-- Run through the entries in temporary temp_tbs_assignments table to create all the remote IPA records
-- Call create_remote_tbs_record() for each, pulling matched records from temp_player_infos and temp_player_contacts.
-- Temporary tables are used, since they will already be populated with data from the Zeus server via CSV files
CREATE OR REPLACE FUNCTION create_all_remote_tbs_records() returns INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
	tbs_record RECORD;
...
-- Run tests with:
-- select tbs.create_remote_tbs_record(364648868, (select pi from player_infos pi where master_id = 105029 limit 1), ARRAY(select pi from player_contacts pi where master_id = 105029), ARRAY(select pi from addresses pi where master_id = 105029) );
...
CREATE OR REPLACE FUNCTION create_remote_tbs_record(match_tbs_id BIGINT, new_player_info_record player_infos, new_player_contact_records player_contacts[], new_address_records addresses[]) returns INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
	found_ipa record;
