...
CREATE OR REPLACE VIEW ipa_ops.ipa_tms_reviews AS
 ...
 WITH tms AS (
    SELECT rank() over (partition by master_id order by id desc) r, * from ipa_ops.ipa_ps_tms_tests
 ),
 mri AS (
    SELECT rank() over (partition by master_id order by id desc) r, * from ipa_ops.ipa_ps_mris
 ),
 health AS (
    SELECT rank() over (partition by master_id order by id desc) r, * from ipa_ops.ipa_ps_healths
 )
 SELECT
    tms.id,
    tms.master_id,
