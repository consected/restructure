...
module OptionListHelper
  ...
  def app_type_select_current_item(use_current_user: false)
    ...
    elsif filter_params_permitted && filter_params_permitted.first.first == 'app_type_id' && object_instance&.app_type_id.blank?
      { selected: filter_params_permitted.first.last }
    else
      {}
    end
  end
