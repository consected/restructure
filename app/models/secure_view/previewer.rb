module SecureView
  class Previewer
    ...
    def cache_key(type)
      "SecureView-doc-#{orig_path}-#{self.class.file_type}-#{type}"
    end
    ...
    def office_docs_to(type, conv_attempts: 0)
      ...
      if previewable? || (pdf? && type != 'pdf')
        ...
        self.path = Rails.cache.fetch(cache_key('renderedpath')) do
          ...
          res = system(SecureView::Config.libreoffice_path, '--headless', '--convert-to', type, '--outdir', temp_dir, orig_path, out: File::NULL, err: File::NULL)
          raise GeneralException, "Failed to convert file to #{type}" unless res

          conv_attempts += 1
          self.path = File.join(temp_dir, self.class.change_extension(File.basename(orig_path), type))
        end
    ...
    def dicom_to_jpg(conv_attempts: 0)
      ...
      if viewable_dicom?
        ...
        self.path = Rails.cache.fetch(cache_key('renderedpath')) do
          create_temp_dir
          self.path = File.join(temp_dir, self.class.change_extension(File.basename(orig_path), 'jpg'))

          res = system(SecureView::Config.dcmj2pnm_path, '+oj', '+Wh', '1', orig_path, path, out: File::NULL, err: File::NULL)
