...
module MasterSearchHandler
  ...
  PlayerInfoRankOrderClause = Arel.sql <<~END_SQL
    case when rank is null then -1000
  ...
  TrackerEventOrderClause = Arel.sql  <<~END_SQL
    protocols.position asc, event_date DESC NULLS last, trackers.updated_at DESC NULLS last#{' '}
  END_SQL

  TrackerHistoryEventOrderClause = Arel.sql 'event_date DESC NULLS last, tracker_history.updated_at DESC NULLS last '

  SimplePlayerJoin = Arel.sql <<~END_SQL
    LEFT JOIN player_infos on masters.id = player_infos.master_id
    LEFT JOIN pro_infos as pro_infos on masters.id = pro_infos.master_id
  END_SQL

  NotTrackerJoin = :no_join
  NotTrackerHistoryJoin = :no_join
  ...
  AltConditions = {
    player_infos: {
      ...
      younger_than: {
        value: :years,
        condition:
          'player_infos.birth_date is not null  AND ((current_date - interval ? )) < player_infos.birth_date'
      },
      older_than: {
        value: :years,
        condition:
    ...
    pro_infos: {
      ...
      middle_name: { value: :starts_with },
      nick_name: { value: :starts_with },
      less_than_career_years: {
        value: :is,
        condition:
          'pro_infos.start_year is not null AND pro_infos.end_year IS NOT NULL ' \
          'AND (pro_infos.end_year - pro_infos.start_year) < ?'
      },
      more_than_career_years: {
        value: :is,
        condition:
          'pro_infos.start_year is not null AND pro_infos.end_year IS NOT NULL ' \
          'AND (pro_infos.end_year - pro_infos.start_year) > ?'
    ...
    player_contacts: {
      data: {
        value: %i[starts_with strip_non_alpha_numeric],
    ...
    not_trackers: {
      protocol_event_id: {
        value: :is,
        condition:
          'NOT EXISTS (select NULL from trackers t_inner where t_inner.protocol_event_id EQUALS_OR_IS_NULL ? AND ' \
          't_inner.sub_process_id = :not_trackers_sub_process_id  AND t_inner.master_id = masters.id)',
        joins: NotTrackerJoin
      },
    ...
    not_tracker_histories: {
      protocol_event_id: {
        ...
        condition:
          'NOT EXISTS ' \
          '(select NULL from tracker_history th_inner where th_inner.protocol_event_id EQUALS_OR_IS_NULL ? ' \
          'AND th_inner.sub_process_id = :not_tracker_histories_sub_process_id AND th_inner.master_id = masters.id)',
        joins: NotTrackerHistoryJoin
      },
      sub_process_id: {
        value: :is,
        condition: 'TRUE',
    ...
    general_infos: {
      first_name: {
        value: :starts_with,
        condition:
          '(player_infos.first_name LIKE ? OR pro_infos.first_name LIKE ? ' \
          'OR player_infos.nick_name LIKE ? OR pro_infos.nick_name LIKE ?)',
        joins: SimplePlayerJoin
      },
      last_name: {
        value: :is,
        condition: '(player_infos.last_name = ? OR pro_infos.last_name = ?)',
        joins: SimplePlayerJoin
      },
      birth_date: {
        value: :is,
        condition: '(player_infos.birth_date = ? OR pro_infos.birth_date = ?)',
        joins: SimplePlayerJoin
      },
      death_date: {
        value: :is,
        condition: '(player_infos.death_date = ? OR pro_infos.death_date = ?)',
        joins: SimplePlayerJoin
      },
      start_year: {
        value: :is,
        condition: '(player_infos.start_year = ? OR pro_infos.start_year = ?)',
        joins: SimplePlayerJoin
      },
      end_year: {
        value: :is,
        condition: '(player_infos.end_year = ? OR pro_infos.end_year = ?)',
        joins: SimplePlayerJoin
      },
      college: {
        value: :is,
        condition: '(player_infos.college = ? OR pro_infos.college = ?)',
        joins: SimplePlayerJoin
      },
      contact_data: {
        value: %i[starts_with strip_non_alpha_numeric],
  ...
  included do
    ...
    # Nested attributes for advanced search form. These items will be extended dynamically by
    # the external_id models when they are initially configured
  ...
  class_methods do
      ...
      params.each do |params_key, params_val|
        if params_val.is_a? Hash
          ...
          if params_key.to_s.include? '_attributes'
            ...
            # logger.debug "Source Reflection: #{r.source_reflection&.name}"
    ...
    # Note that this sorts first, based on the Master rank, which is
    # calculated through a trigger from player info accuracy
