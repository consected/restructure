ActiveSupport.on_load(:nfs_store_config) do
  ...
  if Rails.env.production?
    ...
    # Temporary directory used during upload and download operations.
    # Consider using /var/tmp rather than /tmp (which probably is a tmpfs and resides in memory)
    # since we are catering for very large files and don't want tmpfs to force swapping
