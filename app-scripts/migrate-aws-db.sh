...
if [ "$TEMP_ENV" == 'filestore-production' ]; then
  TEMP_DBNAME=fphs
  DB_SEARCH_PATH='filestore,filestore_admin,ipa_ops,ml_app,ref_data'
  TEMP_HOSTNAME='fphs-aws-db-prod01.c9dljdsduksr.us-east-1.rds.amazonaws.com'
fi

if [ "$TEMP_ENV" == 'zeus-production' ]; then
  TEMP_DBNAME=fphs
  DB_SEARCH_PATH='ml_app,ref_data'
...
if [ "$TEMP_ENV" == 'athena-stage' ]; then
  ...
  DB_SEARCH_PATH='filestore,filestore_admin,ml_app,ref_data'
  TEMP_HOSTNAME='fphs-aws-db-prod01.c9dljdsduksr.us-east-1.rds.amazonaws.com'
fi
